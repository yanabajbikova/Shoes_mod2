...
# Функция для получения логотипа (ищет в import и корне)
...
# Функция для поиска заглушки picture.png
def get_placeholder_pixmap(w, h):
    ...
    # Если даже picture.png нет, тогда рисуем серый квадрат (крайний случай)
...
class OrdersWindow(QtWidgets.QDialog):
    def __init__(self, db):
        super().__init__()
        self.setWindowTitle("Просмотр заказов — ООО Обувь") # Полезный заголовок
        self.resize(900, 500)
        ...
        try:
            ...
            # Цикл заполнения (чтобы таблица не была пустой)
...
class ProductItem(QtWidgets.QFrame):
    def __init__(self, p, parent_win):
        ...
        info.addWidget(QtWidgets.QLabel(f"На складе: {qty} шт."))
